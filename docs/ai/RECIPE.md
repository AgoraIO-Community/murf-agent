...
stable_contracts:
  - id: env.required
    summary: AGORA_APP_ID, AGORA_APP_CERTIFICATE, SARVAM_API_KEY, and MURF_API_KEY are required by FastAPI; AGENT_BACKEND_URL is required by deployed web rewrites.
  - id: api.core-routes
...
- Fixed Sarvam STT, managed OpenAI, and Murf TTS configuration.
...
| `agent.managed-config` | `server/src/agent.py` | Change `ADA_PROMPT`, `AGENT_GREETING`, `turn_detection`, `SarvamSTT`, `OpenAI`, `MurfTTS`, `parameters`, or session options. | Run backend compile and local FastAPI smoke checks; document new env vars in `server/.env.example`. |
...
- The fixed pipeline and its credentials stay server-side; the browser has no runtime selector.

## Stable Contracts

| Contract | Stable shape |
| -------- | ------------ |
| Required backend env | `AGORA_APP_ID`, `AGORA_APP_CERTIFICATE`, `SARVAM_API_KEY`, `MURF_API_KEY` |
| Optional backend env | `SARVAM_LANGUAGE`, `MURF_VOICE_ID`, `AGENT_GREETING`, `PORT` |
