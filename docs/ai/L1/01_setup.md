...
| `SARVAM_API_KEY`         | Python (server)      | Yes      | Server-only Sarvam STT authentication.                                 |
| `SARVAM_LANGUAGE`        | Python (server)      | No       | Recognition language; default `en-IN`.                                 |
| `MURF_API_KEY`           | Python (server)      | Yes      | Server-only Murf TTS authentication.                                   |
| `MURF_VOICE_ID`          | Python (server)      | No       | Murf voice; default `Matthew`.                                         |
| `AGENT_GREETING`         | Python (server)      | No       | Optional first utterance.                                             |
| `PORT`                   | Python (server)      | No       | Default `8001` (`server.py`).                                          |
...
bun run dev:frontend           # cd web && AGENT_BACKEND_URL=http://localhost:8001 bun run dev
