...
4. Implement `server/src/agent.py` with an `Agent` class that reads env once, constructs `AsyncAgora`, builds the fixed `SarvamSTT` + managed `OpenAI` + `MurfTTS` pipeline, starts async sessions, stores sessions by `agent_id`, and stops by active session or `client.stop_agent`.
