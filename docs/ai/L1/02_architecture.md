...
```
   ...
   Agora media + RTM cloud                                       (Sarvam STT + managed OpenAI + Murf TTS)
```
...
3. `ConversationComponent` mounts inside a dynamic `AgoraRTCProvider` (RTC client in `useRef` for StrictMode safety) and:
   ...
   - The transcript normalization layer separates interim, final, and interrupted turns, normalizes timestamps, and preserves confidence when the provider payload includes it.
...
If `AGENT_BACKEND_URL` is unset/empty, **no rewrites register** — the client cannot reach the backend. `bun run dev` always exports `AGENT_BACKEND_URL=http://localhost:8001` before starting Next.
...
## Fixed Pipeline (server/src/agent.py)

| Stage | Implementation |
| ----- | -------------- |
| STT | Sarvam, `SARVAM_LANGUAGE=en-IN` by default |
| LLM | managed OpenAI `gpt-4o-mini` |
| TTS | Murf `Matthew`, `FALCON`, `en-US`, 24 kHz |

The browser cannot select or override these stages. `web/src/lib/pipeline.ts` is an immutable display descriptor only.
