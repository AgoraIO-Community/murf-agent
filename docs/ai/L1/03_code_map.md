...
web/                      # Next.js 16 app (workspace member)
  ...
  src/
    components/
      ...
      QuickstartTranscriptPanel.tsx # Live statuses, timestamps, confidence, copy, JSON export
    ...
    lib/
      ...
      pipeline.ts         # Immutable fixed-pipeline display descriptor
...
| `web/src/components/LandingPage.tsx`                | Session bootstrap, RTM login, and renewal handler.                       |
| `web/src/components/ConversationComponent.tsx`      | RTC join, `AgoraVoiceAI` init, transcript/state/metrics, mic UI.         |
| `web/src/lib/conversation.ts`                       | Transcript UID/text/time/status/confidence normalization and visualizer state mapping. |
...
| `server/src/agent.py`                               | `Agent` class — fixed Murf pipeline + async session lifecycle.           |
