...
| Scope                  | Variable                                  |
| ---------------------- | ----------------------------------------- |
| Python server (required) | `AGORA_APP_ID`, `AGORA_APP_CERTIFICATE`, `SARVAM_API_KEY`, `MURF_API_KEY` |
| Python server (optional) | `SARVAM_LANGUAGE`, `MURF_VOICE_ID`, `AGENT_GREETING`, `PORT` |
...
agent = (
    AgoraAgent(...)
    .with_stt(SarvamSTT(api_key=os.getenv("SARVAM_API_KEY"), language="en-IN"))
    .with_llm(OpenAI(model="gpt-4o-mini", ...))
    .with_tts(MurfTTS(key=os.getenv("MURF_API_KEY"), voice_id="Matthew",
                      model="FALCON", locale="en-US", sample_rate=24000))
)
...
The transcript UI treats confidence as optional. It checks normalized transcript metadata for `confidence`, `confidence_score`, `probability`, or word-level confidence values and omits the score when none is available.
