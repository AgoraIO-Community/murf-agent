...
- **STT:** change the `SarvamSTT(...)` constructor.
- **TTS:** change the `MurfTTS(...)` constructor (`voice_id`, `model`, `locale`, `sample_rate`).
...
- **Pipeline labels:** keep the immutable descriptor in `web/src/lib/pipeline.ts` aligned with the server configuration.
...
- **Backend (FastAPI):** install deps from `server/requirements.txt`, set Agora credentials plus `SARVAM_API_KEY` and `MURF_API_KEY`, then run `python3 server/src/server.py` or `uvicorn server.src.server:app --host 0.0.0.0 --port $PORT`.
...
#   dev:frontend → cd web && AGENT_BACKEND_URL=http://localhost:8001 bun run dev
# }
```
