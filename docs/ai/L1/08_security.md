...
- The FastAPI server is the only process that holds `AGORA_APP_CERTIFICATE`, `SARVAM_API_KEY`, and `MURF_API_KEY`.
...
| FastAPI        | Agora credentials, Sarvam/Murf keys and defaults, `AGENT_GREETING`, `PORT` |

Mark `AGORA_APP_CERTIFICATE` and every speech-provider API key as sensitive secrets in whichever host runs the Python service. None of these values appear in `web/`.
