...
This guide is for coding agents making changes in `agora-murf-voice-agent`.
...
- Agent config: fixed Sarvam STT, managed OpenAI `gpt-4o-mini`, and Murf TTS pipeline
...
- Root scripts start FastAPI on `http://localhost:8001` and Next.js on `http://localhost:3000`.
- The web app calls `/api/*`; Next rewrites those requests to the Python service through `AGENT_BACKEND_URL=http://localhost:8001`.
...
- Keep the standalone Murf pipeline fixed unless the project requirements explicitly change it.
...
- Keep `SARVAM_API_KEY` and `MURF_API_KEY` server-only and required for the fixed pipeline.
