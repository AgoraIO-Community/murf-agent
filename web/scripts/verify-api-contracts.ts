...
async function verifyRewriteContract() {
  ...
  process.env.AGENT_BACKEND_URL = 'http://localhost:8001/'

  try {
    const rewrites = await getRewrites()
    assert(
      rewrites.some(
        (rewrite) => rewrite.source === '/api/get_config' && rewrite.destination === 'http://localhost:8001/get_config',
      ),
      'next.config.ts should rewrite /api/get_config to /get_config on the Python backend',
    )
    assert(
      rewrites.some(
        (rewrite) => rewrite.source === '/api/startAgent' && rewrite.destination === 'http://localhost:8001/startAgent',
      ),
      'next.config.ts should rewrite /api/startAgent to /startAgent on the Python backend',
    )
    assert(
      rewrites.some(
        (rewrite) => rewrite.source === '/api/stopAgent' && rewrite.destination === 'http://localhost:8001/stopAgent',
      ),
...
async function verifyApiClientRequests() {
  ...
  globalThis.fetch = (async (input, init) => {
    ...
    if (url.pathname === '/api/startAgent') {
      ...
      assert(Object.keys(body).length === 3, 'POST /api/startAgent should include only channel and UID fields')
