...
- Local/Test/Prod differences: root `bun run dev` exports `AGENT_BACKEND_URL=http://localhost:8001`; deploys must set it on the web host
- Entry point: `web/app/page.tsx` renders `LandingPage`
- Default port: 3000 for Next.js, 8001 for FastAPI
...
- Third-party services: Sarvam STT, Agora-managed OpenAI `gpt-4o-mini`, and Murf TTS
- Showcase transcript: interim/final state, timestamps, optional confidence, auto-scroll, clipboard copy, and JSON download
