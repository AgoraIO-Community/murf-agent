...
function getCta(issue: ConnectionIssue): string | null {
  ...
  if (msg.includes('http 401') || code === '401') {
    if (msg.includes('asr')) return 'Verify the Sarvam API key.'
    if (msg.includes('tts')) return 'Verify the Murf API key.'
    return 'Verify the failing pipeline stage credentials.'
  }
  if (msg.includes('http 403') || code === '403') return 'Verify Sarvam or Murf API permissions.'
  if (msg.includes('http 404') || code === '404') return 'Verify the configured provider endpoint and model.'
  ...
  if (msg.includes('http 500') || msg.includes('http 502') || msg.includes('http 503') || msg.includes('http 504')) {
    return 'Retry shortly and check the failing provider service status.'
  }
  if (msg.includes('websocket') && msg.includes('reject')) {
    return 'Verify Sarvam or Murf authentication and endpoint access.'
  }
