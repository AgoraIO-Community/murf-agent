...
import { type QuickstartAgentMetric, QuickstartPipelineMetrics } from '@/components/QuickstartPipelineMetrics'
...
export default function ConversationComponent({
  ...
  const isMicReady = localMicrophoneTrack !== null
  const sessionStatus = useMemo(
    ...
    [agentState, connectionState, isAgentConnected, isEnabled, isMicReady],
  )
  ...
  const handleMicEnabledChange = useCallback(
    (enabled: boolean) => {
      if (localMicrophoneTrack) setIsEnabled(enabled)
    },
  ...
  return (
    <QuickstartConversationLayout
      ...
      visualizer={
        <section
          className="relative flex min-h-[9rem] w-full items-center justify-center"
      ...
      controls={
        <fieldset
          disabled={!isMicReady}
          className="flex w-full items-center justify-between gap-3 rounded-2xl border border-border bg-background/90 px-3 py-2.5 shadow-sm disabled:cursor-not-allowed disabled:opacity-60"
          ...
          <div className="flex min-w-0 items-center gap-3">
            <div className="conversation-mic-host flex items-center justify-center">
              <MicButtonWithVisualizer
                isEnabled={isEnabled}
                setIsEnabled={handleMicEnabledChange}
                track={localMicrophoneTrack}
                onToggle={handleMicToggle}
                className="overflow-visible"
                aria-label={
                  isMicReady ? (isEnabled ? 'Mute microphone' : 'Unmute microphone') : 'Microphone unavailable'
                }
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold text-foreground">
                {isMicReady ? (isEnabled ? 'Microphone ready' : 'Microphone muted') : 'Microphone unavailable'}
              </p>
              <p className="mt-0.5 truncate text-[10px] text-muted-foreground">
                {isMicReady ? 'Local audio track initialized' : 'Waiting for device permission'}
              </p>
            </div>
          </div>
